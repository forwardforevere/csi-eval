"""Command-line interface: ``csieval``.

Usage::

  # Full form
  csieval run --task eigenvector_feedback \\
               --checkpoint runs/best.pt \\
               --data-root /path/to/data \\
               --output results/eval

  # From YAML config
  csieval run --config experiment.yaml

  # Compare
  csieval compare --checkpoints run1.pt run2.pt --names A B \\
                  --output results/compare

  # Discovery
  csieval list --kind all
"""
...
from csieval.version import __version__
...
Examples:
  # Minimal
  csieval run --task eigenvector_feedback --checkpoint run.pt --data-root ./data

  # Custom model class
  csieval run --task eigenvector_feedback --checkpoint run.pt \\
              --model-class models.CsiNet --model-kwargs '{"nt":32,"n_subbands":13}'

  # From YAML config
  csieval run --config eval_config.yaml

  # Compare two models
  csieval compare --checkpoints run_a.pt run_b.pt --names A B --output results/compare
        """,
    )
    ...
    run_p.add_argument(
        ...
        help=(
            ...
            "Default: ~/.cache/csieval/ (Linux/macOS). "
...
def _load_config(args: argparse.Namespace) -> Any:
    ...
    from csieval import EvalConfig
...
def _cmd_run(args: argparse.Namespace) -> int:
    ...
    if args.validate_only:
        ...
        print(f"[csieval] Config validated.")
    ...
    info = ev.info()
    print(f"[csieval] task={info['config']['task']}  "
...
def _cmd_compare(args: argparse.Namespace) -> int:
    from csieval import Evaluator, EvalConfig
    from csieval.core.report import EvalReport
...
def _cmd_list(args: argparse.Namespace) -> int:
    from csieval.core.registries import MetricRegistry, TaskRegistry

    if args.kind in ("tasks", "all"):
        try:
            from csieval.tasks import eigenvector_feedback  # noqa: F401
        except Exception:
            pass
    ...
    if args.kind in ("metrics", "all"):
        from csieval import metrics as _m  # noqa: F401
...
def main(argv: List[str] | None = None) -> int:
    ...
    try:
        ...
        print("\n[csieval] Interrupted by user.")
        return 130
    except Exception as e:
        print(f"[csieval] Error: {e}", file=sys.stderr)
        import traceback
